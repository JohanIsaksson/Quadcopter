...
void altitude_tester(){
  ...
  imu_update_horizon(&im, time_diff);

  double x = sin(im.ypr_rad[2]);
  double y = sin(im.ypr_rad[1]);
  double z = x*x + y*y;

  double a_z = im.azs/sqrt(1.0 - z)*9.81; //vertical accerleration with correction
  ...
  Serial.print("\t\t");
  Serial.println(a_z, 5); //Serial.print(",");

  while(micros() < time_last + 5000);
